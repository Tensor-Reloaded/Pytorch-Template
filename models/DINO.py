...
class Dino(nn.Module):
    def __init__(
        ...
        try:
            model = getattr(models, transformer_name)
        except:
            print(f"This model is not implemented ({transformer_name}), go ahead and commit it")
            exit()

        transformer_params = dict((key,d[key]) for d in transformer_params for key in d)
        self.net = model(**transformer_params)
        ...
        # TODO see what augmentation to use here, maybe use augmentations defined in config files for this and pass the name of it as a parameter
        DEFAULT_AUG = transforms.Compose([
            transformations['RandAugment'](1, 8, 0.5),
        ])
        ...
        self.local_crop = T.RandomResizedCrop((image_height, image_width), scale = (0.05, local_upper_crop_scale))
        self.global_crop = T.RandomResizedCrop((image_height, image_width), scale = (global_lower_crop_scale, 1.))

        self.student_encoder = NetWrapper(self.net, num_classes_K, projection_hidden_size, projection_layers, layer = hidden_layer)
        ...
        self.forward(torch.randn(2, 1, image_height, image_width, device=device))
